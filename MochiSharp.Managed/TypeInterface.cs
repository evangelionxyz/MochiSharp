...
internal static class TypeInterface
{
    ...
    internal static Type? FindType(int assemblyLoadContextId, string? typeName)
    {
        var type = Type.GetType(typeName!,(name) =>
            {
                AssemblyLoader.s_AssemblyContexts.TryGetValue(assemblyLoadContextId, out var assemblyContext);
                return AssemblyLoader.ResolveAssembly(assemblyContext, name);
    ...
    internal static unsafe void GetAssemblyTypes(int InAssemblyLoadContextId, int InAssemblyId, int* OutTypes, int* OutTypeCount)
    {
        try
        {
            ...
            {
                ManagedHost.LogMessage($"Couldn't get types for assembly '{InAssemblyId}', assembly not found.", MessageLevel.Error);
                return;
            }

            if (assembly == null)
            {
                ManagedHost.LogMessage($"Couldn't get types for assembly '{InAssemblyId}', assembly was null.", MessageLevel.Error);
                return;
            }
    ...
    internal static unsafe void GetAttributeFieldValue(int InAttribute, NativeString InFieldName, IntPtr OutValue)
    {
        try
        {
            ...
            if (fieldInfo == null)
            {
                ManagedHost.LogMessage($"Failed to find field with name '{InFieldName}' in attribute {targetType.FullName}.", MessageLevel.Error);
                return;
            }
