...
#include <MochiSharp/GC.hpp>
#include <MochiSharp/Array.hpp>
#include <MochiSharp/Attribute.hpp>
...
static void PrintStringIcall(mochi::String InString)
{
    std::cout << std::string(InString) << std::endl;
}

static void NativeArrayIcall(mochi::Array<float> InValues)
{
...
static mochi::Array<float> ArrayReturnIcall()
{
    std::cout << "ArrayReturnIcall" << std::endl;
    return mochi::Array<float>::New({ 10.0f, 5000.0f, 1000.0f });
}

int main([[maybe_unused]] int argc, char **argv)
{
    auto exeDir = std::filesystem::path(argv[0]).parent_path();
    auto mochiDir = exeDir.string();
    mochi::HostSettings settings;
    settings.MochiSharpDirectory = mochiDir;
    settings.ExceptionCallback = ExceptionCallback;
    mochi::HostInstance hostInstance;
    ...
    auto arr = mochi::Array<float>::New({ 5.0f, 0.0f, 10.0f, -50.0f });
    exampleInstance.InvokeMethod("ArrayDemo", arr);
    mochi::Array<float>::Free(arr);
