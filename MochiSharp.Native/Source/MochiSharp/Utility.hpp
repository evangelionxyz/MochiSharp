#pragma once
#ifndef MOCHI_UTILITY_HPP
#define MOCHI_UTILITY_HPP
...
namespace mochi
{
	...
	constexpr ManagedType GetManagedType()
	{
		...
		else if constexpr (std::is_same_v<TArg, std::string> || std::is_same_v<TArg, mochi::String>)
	...
	/*
	 * TODO: Work out a way to allow method invoke to use C++-y types (i.e. `std::string` instead of `mochi::String`).
